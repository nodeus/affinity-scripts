...
function makeStrokeGradientTransform(node) {
  try {
    const bbox = node.baseBox;
    if (!bbox) return null;

    const width = Math.max(bbox.width, bbox.height, 1);

    const tb = new TransformBuilder();
    tb.scaleXY(width, width);
    tb.translateXY(bbox.x, bbox.y + bbox.height / 2);
...
function getNodeStrokeInfo(node) {
  try {
    const fd = getPenFillDescriptor(node);
    if (!fd) return null;
...
function makePenFillDescriptorForNode(entry, rawFill, nodeEntry) {
  ...
  if (ftv === FillType.Gradient.value) {
    ...
    if (entry.type !== 'gradient') {
      const gradTransform = makeStrokeGradientTransform(nodeEntry.node);
      if (gradTransform) {
...
function createReplaceCommand(doc, pageDialogs) {
  ...
  for (const pd of pageDialogs) {
    for (const picker of pd.pickers) {
      ...
      if (!currentKey) {
        firstErr = firstErr || 'Unsupported replacement stroke fill';
        continue;
      }
      ...
      for (const nodeEntry of e.nodeEntries) {
        try {
          cb.addCommand(DocumentCommand.createSetPenFill(
            ...
            makePenFillDescriptorForNode(e, rawFill, nodeEntry)
          ));
...
for (const n of allNodes) {
  const info = getNodeStrokeInfo(n);
  if (!info) continue;
